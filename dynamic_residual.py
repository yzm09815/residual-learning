...
class ProgressiveResidualWrapper(gym_new.ActionWrapper):
    ...
    def _get_scale(self):
        """调用统一的 scale 计算函数"""
        scale = compute_scale(TRAINING_PROGRESS.progress) #全局计数器（通常是 $Step/Total\_Steps$），代表当前训练的成熟度。
    ...
    def action(self, action_sac):
        ...
        scale = self._get_scale()
         ...
         #action_sac[0] —— 速度修正因子 (Linear Velocity Component) 范围在 [-1, 1] 之间
        # 它对应机器人的纵向运动。
        # 如果它是正值：代表神经网络认为现在的物理规则（APF）太慢了，需要“踩油门”。
        # 如果它是负值：代表神经网络认为 APF 冲得太猛，需要“踩刹车”。

        # action_sac[1] —— 航向角修正因子 (Angular Component)  范围在 [-1, 1] 之间
        # 它对应机器人的横向转动。
        # 如果它是正值：代表机器人需要向左偏航。
        # 如果它是负值：代表机器人需要向右偏航。
...
# 4. 训练主流程
# ==========================================
#工厂函数 负责创建和组装环境。  下面的env=就是把整个结构分为许多层依次运算
def make_env(): 
...
if __name__ == "__main__":
    log_dir = "./sac_apf_logs_v8b_progressive/"
    ...
    print("=" * 60)
    print("🎯 SAC + 渐进式残差 APF (v8b - Two-Phase Residual)")
    print("=" * 60)
    print()
    print("  v8b 设计哲学：先稳后放")
    print("  ─────────────────────────────────────────────")
    print("  公式: final = APF + δ × scale(t)")
    print("  δ = SAC 输出 ∈ [-1, 1]")
    print("  阶段1: t < 15%  → scale = 1.3（固定，等同 v7）")
    print("  阶段2: t ≥ 15%  → scale 从 1.3 线性增长到 2.0")
    print("  ─────────────────────────────────────────────")
    print("  训练进度    scale    残差范围      效果")
    print("  ─────────────────────────────────────────────")
    print("  0~15% 暖身  1.30     ±1.30        = v7，SAC 正常学习")
    print("  35%         1.47     ±1.47        开始放大")
    print("  57%         1.65     ±1.65        SAC 中等扩展")
    print("  78%         1.83     ±1.83        SAC 主导避障")
    print("  100% 结束   2.00     ±2.00        SAC 可完全覆盖")
    print("  ─────────────────────────────────────────────")
    print()
    print("  vs v7：前15%完全一致，之后逐步超越")
    print("  vs v8旧：起始scale=1.3而非0.3，不会卡在0")
    print("=" * 60)
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000,
        save_path=log_dir,
        name_prefix='sac_progressive_v8b'
    )
    ...
    model.save("sac_progressive_v8b_final")
